...
const Quiz = () => {
  const { id } = useParams(); // Get the test ID from URL params
  const [currentQuestion, setCurrentQuestion] = useState(0);
  const [selectedAnswers, setSelectedAnswers] = useState({});
  const [markedQuestions, setMarkedQuestions] = useState([]);
  const [isDirectionsVisible, setIsDirectionsVisible] = useState(false);

  // Debugging: Log the ID received from URL
  console.log('Test ID:', id);

  // Find the section by the ID from URL
  const section = quizData.sections.find((sec) => sec.id === parseInt(id));

  // If the section is not found, show a fallback message
  if (!section) {
    return <div>Section not found</div>;
  }

  // Get the current question from the section
  ...
  // Debugging: Log the selected answers
        ...
        <div className="col-12">
          <div className="card">
            <SectionHeader
              ...
              isMarked={markedQuestions.includes(`${id}-${currentQuestion}`)}
